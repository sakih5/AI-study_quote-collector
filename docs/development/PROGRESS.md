...
- [x] タグ選択・作成
- [x] 一括登録（[+]ボタン）
- [x] カスタムフック（useActivities, useTags）
- [ ] 出典選択
  - [ ] 本（既存選択・新規登録）
  - [ ] SNS（既存選択・新規登録）
  - [ ] その他（自由入力）
...
### ✅ 完了: 認証機能・基本レイアウト・全API実装・フレーズ登録モーダル（部分）

バックエンドAPI実装とフレーズ登録モーダル（セクション1）が完了しました。

**作業ログ**: [2025-10-29_api_and_frontend.md](./work_logs/2025-10-29_api_and_frontend.md)

### フロントエンド実装（推奨順序）

1. **フレーズ登録モーダル完成** - 4〜5時間
   - ✅ セクション1: フレーズ & 分類分け（完了）
   - 🔲 セクション2: 出典選択（本/SNS/その他）
   - 🔲 API連携・バリデーション・エラーハンドリング

2. **ホーム画面フレーズ一覧実装** - 6〜8時間
   - グループ化表示（書籍/SNSユーザー単位）
   - 無限スクロール
   - キーワード検索
   - 活動領域・タグフィルター
   - API連携
