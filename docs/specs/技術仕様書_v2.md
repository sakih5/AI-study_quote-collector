...
- **OCR**: pytesseract（バックエンド）

#### インフラ
- **フロントエンド**: Vercel
- **バックエンド**: Google Cloud Run (asia-northeast1)
- **データベース**: Supabase (クラウドホスト)
- **ドメイン**: Vercel Domains
...
### 3.3 バックエンドOCR実装（pytesseract）

#### 3.3.1 概要

バックエンドではpytesseractを使用してOCR処理を行います。Tesseract.jsに比べて軽量で、Dockerコンテナ内での実行に適しています。
...
def extract_text_from_image(image_bytes: bytes, language: str = 'jpn') -> dict:
    """画像からテキストを抽出"""
    ...
    for i in range(len(data['text'])):
        if int(data['conf'][i]) > 0:
            word = {
                'text': data['text'][i],
                'confidence': float(data['conf'][i]) / 100.0,  # 0-1に正規化
                'bbox': {
                    'x': int(data['left'][i]),
                    'y': int(data['top'][i]),
...
- **フロントエンド（Tesseract.js）**: 現在は使用されていない（将来的な機能拡張用）
- **バックエンド（pytesseract）**: 実際のOCR処理を担当
  - メリット: 軽量、サーバーリソースで処理、依存関係の削減
  - デメリット: ネットワーク通信が必要

#### 3.3.5 パフォーマンス最適化

- 画像の前処理（グレースケール、コントラスト強化）で精度向上
- PSM mode 6（単一テキストブロック）を使用
- OEM mode 1（LSTM neural net）を使用
- 信頼度による単語フィルタリング
